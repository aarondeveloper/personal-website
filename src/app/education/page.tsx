    ...
    <PageTemplate>
      <main className="min-h-screen flex flex-col pt-16">
        <div className="relative flex-1 flex flex-col items-center px-5 mx-auto max-w-[1850px]">
          {/* Header Section */}
          <div className="w-full bg-black/30 backdrop-blur-md rounded-2xl p-8 mb-8 transform hover:scale-[1.02] transition-transform duration-300">
            <div className="flex items-center gap-6">
              ...
              <div className="flex-1">
                <div className="flex justify-between items-start">
                  <div>
                    <h1 className="text-4xl font-bold bg-gradient-to-r from-blue-500 to-yellow-500 bg-clip-text text-transparent">
                      University of Delaware
                    </h1>
                    <h2 className="text-2xl text-white mt-2">B.S. in Computer Science</h2>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-emerald-400 text-xl">GPA: 3.571</span>
                  ...
                  <a
                    href="/transcript.pdf"
                    download
                    className="flex items-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2 rounded-lg transition-all duration-200 hover:scale-105 group"
                  >
                    <svg
          ...
          <div className="w-full mb-8">
            <h2 className="text-3xl font-bold text-white mb-6">Scholarship Awards</h2>
            <div className="flex flex-wrap gap-4">
              {scholarships.map((scholarship) => (
                <div
                  key={scholarship}
                  className="bg-gradient-to-r from-blue-500/20 to-yellow-500/20 backdrop-blur-md rounded-xl p-6 hover:from-blue-500/30 hover:to-yellow-500/30 transition-all duration-300 hover:-translate-y-1"
                >
                  <div className="flex items-center gap-2">
                    <svg className="w-5 h-5 text-yellow-500" fill="currentColor" viewBox="0 0 20 20">
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                    <h3 className="text-xl font-semibold text-white">{scholarship}</h3>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="w-full">
            <h2 className="text-3xl font-bold text-white mb-6">Relevant Coursework</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {courses.map((course) => (
                <div
                  key={course.name}
                  className="group bg-black/30 backdrop-blur-md rounded-xl p-4 hover:bg-black/40 transition-all duration-300 hover:-translate-y-1"
                >
                  <h3 className="text-white text-lg font-semibold group-hover:text-emerald-400 transition-colors">
                    {course.name}
                  </h3>
                  {course.tech && (
                    <span className="inline-block bg-emerald-500/20 text-emerald-400 px-2 py-1 rounded-md text-sm mt-2">
                      {course.tech}
                    </span>
                  )}
                </div>
